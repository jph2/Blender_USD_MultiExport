...
class USDME_Logger:
    ...
    def _get_addon_info(self) -> Dict[str, Any]:
        ...
        try:
            # Try the correct module name first
            addon_module = sys.modules.get('blender_usd_multiexport_addon')
            if addon_module and hasattr(addon_module, 'bl_info'):
                bl_info = dict(addon_module.bl_info)
                # Format version as string for clarity
                if 'version' in bl_info:
                    v = bl_info['version']
                    bl_info['version_string'] = f"{v[0]}.{v[1]}.{v[2]}"
                return bl_info
            ...
            if 'version' in result:
                v = result['version']
                result['version_string'] = f"{v[0]}.{v[1]}.{v[2]}"
            return result
        except Exception as e:
            # Last resort: return hardcoded version from this file's package
            try:
                from . import __init__ as init_module
                if hasattr(init_module, 'bl_info'):
                    bl_info = dict(init_module.bl_info)
                    if 'version' in bl_info:
                        v = bl_info['version']
                        bl_info['version_string'] = f"{v[0]}.{v[1]}.{v[2]}"
                    return bl_info
                ...
                pass
            return {"error": f"Could not retrieve addon info: {str(e)}"}
