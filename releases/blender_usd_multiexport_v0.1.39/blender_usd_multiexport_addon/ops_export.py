...
class USDME_OT_export_start_points(Operator):
    ...
    def _apply_stage_metadata(self, filepath: str, start_point, logger, start_point_context: dict) -> None:
        ...
        try:
            ...
            # Open the USD stage for editing
            stage = Usd.Stage.Open(filepath)
            if not stage:
                logger.log_warning(
                    f"Could not open USD stage for metadata update: {filepath}",
                    context=start_point_context
                )
                return
            ...
            up_axis = UsdGeom.Tokens.y if y_is_up else UsdGeom.Tokens.z
            
            # Apply stage metadata
            # v0.1.39: IMPORTANT - We always set metersPerUnit = 1.0 (meters) regardless of target_unit
            # This avoids double-conversion issues with Omniverse:
            # - If we set metersPerUnit = 0.01 (cm) AND apply geometric scale, Omniverse would
            #   ALSO apply unit conversion (0.01x), cancelling out our scale transform.
            # - By keeping metersPerUnit = 1.0, we let Omniverse's scene-level settings handle
            #   any unit conversion, which is the correct behavior.
            # - The target_unit setting remains available for future use (e.g., if users need
            #   to export to tools that don't do automatic unit conversion).
            UsdGeom.SetStageMetersPerUnit(stage, 1.0)  # Always meters - let Omniverse handle conversion
            UsdGeom.SetStageUpAxis(stage, up_axis)
            
            source_unit = getattr(start_point, 'source_unit', 'METERS')
            
            rotation_applied = False
            scale_applied = False  # v0.1.39: Scale disabled to avoid double-conversion
            
            # Apply Y-up rotation to root prim if needed
            # v0.1.39: Removed geometric scale - only rotation is applied
            if y_is_up:
                ...
                if default_prim and default_prim.IsValid():
                    ...
                    if xformable:
                        ...
                        # Apply Y-up rotation (-90° around X axis)
                        # This converts from Blender's Z-up to Y-up coordinate system
                        rotate_op = xformable.AddRotateXYZOp(UsdGeom.XformOp.PrecisionDouble)
                        rotate_op.Set(Gf.Vec3d(-90.0, 0.0, 0.0))
                        rotation_applied = True
                        logger.log_step("rotation_transform_applied", {
                            **start_point_context,
                            "rotation": [-90.0, 0.0, 0.0],
                            "reason": "Z-up to Y-up conversion",
                        })
            
            # Save the changes
            stage.Save()
            
            logger.log_step("stage_metadata_applied", {
                **start_point_context,
                "meters_per_unit": 1.0,  # v0.1.39: Always meters
                "meters_per_unit_note": "Always 1.0 (meters) to avoid Omniverse double-conversion",
                "source_unit": source_unit,
                "target_unit": target_unit,
                "scale_applied": scale_applied,  # Always False in v0.1.39
            ...
            logger.log_warning(
                "USD Python API (pxr) not available. Stage metadata not updated.",
                context=start_point_context
            )
        except Exception as e:
            logger.log_error(
