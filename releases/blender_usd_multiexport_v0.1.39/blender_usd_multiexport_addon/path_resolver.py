...
class PathResolver:
    ...
    def resolve_export_path(self, relative_path: str, create_directories: bool = True) -> Optional[str]:
        ...
        try:
            ...
            # v0.1.31: Sanitize filepath to remove invalid characters (tabs, etc.)
    ...
    def sanitize_filepath(self, filepath: str) -> str:
        """
        Sanitize a filepath by removing or replacing invalid characters.
        
        v0.1.31: Handles tab characters and other control characters that
        can cause USD export to fail.
        
        Args:
            filepath: The filepath to sanitize
            
        Returns:
            Sanitized filepath string
        """
        if not filepath:
            return filepath
        ...
        # Replace control characters (tabs, newlines, etc.) with underscores
        # Control characters are ASCII 0-31 and 127
        # \t (tab) is ASCII 9, \n (newline) is ASCII 10, \r (carriage return) is ASCII 13
        sanitized_filename = re.sub(r'[\x00-\x1f\x7f]', '_', filename)
        
        # Also replace other problematic characters for Windows filesystems
        # < > : " / \ | ? *
        ...
        if sanitized_filename != filename:
            self.logger.log_warning(
                f"Filename sanitized: '{filename}' -> '{sanitized_filename}'",
                suggestion="Consider renaming object/collection to avoid invalid characters"
            )
        
        # Reconstruct path
        if directory:
            return os.path.join(directory, sanitized_filename).replace('\\', '/')
    ...
    def _validate_path(self, path: str) -> bool:
        ...
        # Check for invalid characters in path (including control characters)
        # v0.1.31: Added tab and other control characters
        invalid_chars = ['<', '>', '|', '"', '*', '?', '\t', '\n', '\r']
        if any(char in path for char in invalid_chars):
            self.logger.log_warning(
                f"Path contains invalid characters: {repr(path)}",
                suggestion="Avoid using < > | \" * ? and control characters (tab, newline) in file paths"
            )
            return False
