...
class ScopedIsolation:
    ...
    Safely caches current selection/visibility state, isolates the target endpoint,
    ...
    def _isolate_target(self):
        """Isolate the target endpoint by hiding everything else and selecting only the target."""
        scene = self.context.scene
        view_layer = self.context.view_layer
        ...
        # Determine what objects belong to the target endpoint
    ...
    def _get_target_objects(self) -> List[bpy.types.Object]:
        ...
        elif isinstance(self.target_endpoint, str):
