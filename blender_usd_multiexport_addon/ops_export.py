...
def ensure_modifier_target_visibility(
        context,
        mod,
        visibility_backups,
        logger=None,
        start_point_context=None):
...
def _select_objects_for_ops(context: Context, objects) -> list:
    """Select objects for ops and set an active object safely."""
    if not objects:
        return []
    ...
    for obj in objects:
        try:
            ...
            obj_ref.hide_viewport = False
            obj_ref.hide_set(False)
    ...
    if valid_objects:
        ...
        try:
            if valid_objects[0].mode != "OBJECT":
...
def _apply_collection_quick_normalize(
    context: Context,
    objects,
    start_point,
    logger,
    ...
    """Quick-fix collection normalization using Blender ops on duplicates only."""
    normalize_position = getattr(start_point, "normalize_position", False)
    # Normalize Scale/Rotation disabled (upcoming): breaks collection decals — see 04_Implementation_Plan.md
    normalize_rotation = False
    normalize_scale = False
    pivot_only = getattr(start_point, "collection_pivot_only", False)
    pivot_enabled = _collection_pivot_enabled(start_point)

    if not (pivot_only or normalize_position or normalize_rotation or normalize_scale or pivot_enabled):
        return False

    targets = _select_objects_for_ops(context, objects)
    if not targets:
        if logger:
            logger.warning(
                "Collection normalization requested but no valid targets were available.",
                extra=start_point_context,
            )
        return False

    cursor = context.scene.cursor if context and context.scene else None
    cursor_backup = cursor.location.copy() if cursor else None

    try:
        if pivot_only or pivot_enabled:
            pivot_vector = _get_collection_pivot_vector(start_point, context)
            if pivot_vector is not None and cursor:
                cursor.location = pivot_vector
                _select_objects_for_ops(context, targets)
                try:
                    ...
                    if logger:
                        logger.warning(
                            f"Origin set failed during collection normalization: {exc}",
                            extra=start_point_context,
                        )
                else:
                    if logger:
                        logger.log_step("collection_pivot_applied", {
                            **start_point_context,
                            "pivot_source": getattr(start_point, "collection_pivot_source", "NONE"),
                            "pivot_vector": (pivot_vector.x, pivot_vector.y, pivot_vector.z),
                            "targets": len(targets),
                        })
            elif pivot_only and logger:
                logger.warning(
                    "Pivot-only enabled but no pivot source resolved; origin set skipped.",
                    extra=start_point_context,
                )

        if (normalize_position or normalize_rotation or normalize_scale) and not pivot_only:
            _select_objects_for_ops(context, targets)
            try:
                ...
                if logger:
                    logger.warning(
                        f"Transform apply failed during collection normalization: {exc}",
                        extra=start_point_context,
                    )
            else:
                if logger:
                    logger.log_step("collection_transforms_applied", {
                        **start_point_context,
        ...
        if cursor and cursor_backup is not None:
...
def _create_fake_parent_for_collection(
    ...
    # instead of Blender parenting, which has complex matrix_parent_inverse
    # behavior.
    ...
    # v0.1.64 FIX: Ensure fake_parent's matrix_world is fully evaluated before
    # parenting
    ...
    # Only update matrix_parent_inverse if goal is "keep children fixed while
    # parent changes" (NOT our use case)

    # Force view layer update after normalization to ensure transforms are
    # current
    context.view_layer.update()
...
def _bake_fake_parent_transform_to_children(
    ...
    # context.view_layer.update() is often not enough for freshly modified
    # transforms
    ...
    for obj in child_objects:
        ...
        try:
            ...
            # Using evaluated_get() ensures matrix is fully computed after all
            # transforms
...
class USDME_OT_export_start_points(Operator):
    ...
    def execute(self, context: Context):
        ...
        if not settings.start_points:
            ...
            self.report(
                {"INFO"},
                "No USD Multi Export start points defined on this scene.")
            logger.end_operation(
                success=False, result_info={
                    "reason": "no_start_points"})
            return {"CANCELLED"}

        try:
            enabled_start_points = [
                ep for ep in settings.start_points if ep.enabled]
            ...
            logger.log_error(
                ...
                exception=e,
                extra={
                    "total_start_points": len(
                        settings.start_points) if settings.start_points else 0},
                recoverable=False)
            self.report({"ERROR"}, f"Failed to process start points: {str(e)}")
            logger.end_operation(
                success=False,
                result_info={
                    "reason": "start_point_filtering_error",
                    "error": str(e)})
            return {"CANCELLED"}
        ...
        # Pre-flight validation: Check all enabled start points for
        # completeness
        ...
        try:
            ...
            for start_point in enabled_start_points:
                ...
                if start_point.start_point_type == 'COLLECTION':
                    if not start_point.collection_name or not start_point.collection_name.strip():
                        ...
                        issues.append(
                            f"collection '{start_point.collection_name}' not found")
                    ...
                    if not start_point.object_name or not start_point.object_name.strip():
                        ...
                        issues.append(
                            f"object '{start_point.object_name}' not found")
            ...
            logger.log_error(
                ...
                exception=e,
                extra={"start_point_count": len(enabled_start_points)},
                recoverable=False
            )
            ...
            logger.end_operation(
                success=False,
                result_info={
                    "reason": "preflight_validation_exception",
                    "error": str(e)})
            return {"CANCELLED"}
        ...
        if invalid_start_points:
            logger.log_step("preflight_validation_failed", {"invalid_start_points": [
                            {"name": name, "issues": issues} for name, issues in invalid_start_points]})
            ...
            logger.log_error(
                ...
                extra={
                    "invalid_start_points": invalid_start_points},
                recoverable=True)
                    ...
                    try:
                        ...
                        if start_point.start_point_type == 'COLLECTION':
                            if not start_point.collection_name or not start_point.collection_name.strip():
                                logger.log_error(
                                    ...
                                    recoverable=True)
                                self.report(
                                    {"WARNING"}, f"Start point '{start_point.name}': No collection specified.")
                                failed_count += 1
                                continue
                        elif start_point.start_point_type == 'OBJECT':
                            if not start_point.object_name or not start_point.object_name.strip():
                                logger.log_error(
                                    ...
                                    recoverable=True)
                                self.report(
                                    {"WARNING"}, f"Start point '{start_point.name}': No object specified.")
                                failed_count += 1
                                continue
                        ...
                        if not start_point.filepath or not start_point.filepath.strip():
                            logger.log_error(
                                ...
                                recoverable=True)
                            self.report(
                                {"WARNING"}, f"Start point '{start_point.name}': No filepath specified.")
                            failed_count += 1
                            continue
                        ...
                        target = None
                        if start_point.start_point_type == 'COLLECTION':
                            target = bpy.data.collections.get(
                            ...
                            if not target:
                                ...
                                self.report(
                                    {"WARNING"},
                                    f"Start point '{start_point.name}': Collection '{start_point.collection_name}' not found.")
                                failed_count += 1
                                continue
                            ...
                            if not target:
                                ...
                                self.report(
                                    {"WARNING"},
                                    f"Start point '{start_point.name}': Object '{start_point.object_name}' not found.")
                                failed_count += 1
                                continue
                        ...
                        filepath = path_resolver.resolve_export_path(
                            start_point.filepath)
                        if not filepath:
                            logger.log_error(
                        ...
                        if start_point.create_subfolder:
                            ...
                            # Get the target name (collection or object) for
                            # filename generation
                            ...
                            if file_dir.name == "USD_StartPoint":
                                # Already in USD_StartPoint folder, just ensure
                                # filename is correct
                                ...
                                # Create subfolder path: USD_StartPoint/ (just
                                # the name, no object/collection name)
                                ...
                                # If filepath is a directory or has no
                                # filename, generate one using target name
                                file_path_obj = Path(filepath)
                                if file_path_obj.is_dir() or not file_path_obj.name or file_path_obj.suffix == '':
                                    # Generate filename using target name
                                    if target_name:
                        ...
                        # Track duplicated objects OUTSIDE ScopedIsolation so
                        # cleanup can happen after isolation exits
                        duplicated_objects_for_cleanup = []
                        # v0.1.14: Track mesh data name swaps for cleanup
                        mesh_swaps_for_cleanup = {}
                        # v0.1.17: Track name swaps for cleanup (OBJECT and
                        # COLLECTION types)
                        name_swaps_for_cleanup = {}

                        # v0.1.17: Clean up any leftover objects from previous failed exports
                        # This prevents accumulation of __USDME_ORIG_* objects
                        # or _dup* duplicates
                        ...
                        for obj_name in list(bpy.data.objects.keys()):
                            if obj_name.startswith("__USDME_ORIG_"):
                                ...
                                if obj:
                                    ...
                                    original_name = obj_name.replace(
                                        "__USDME_ORIG_", "", 1)
                                    # Check if original name is already taken
                                    if original_name not in bpy.data.objects:
                                        try:
                                            obj.name = original_name
                                            leftover_objects_cleaned.append(
                                                f"{obj_name} -> {original_name}")
                                        except BaseException:
                                            pass

                        # Also clean up any _dup* objects that might be left
                        # over
                        for obj_name in list(bpy.data.objects.keys()):
                            if "_dup" in obj_name and obj_name.endswith(
                                    ("_dup1", "_dup2", "_dup3", "_dup4", "_dup5")):
                                obj = bpy.data.objects.get(obj_name)
                                if obj:
                                    try:
                                        bpy.data.objects.remove(obj)
                                        leftover_objects_cleaned.append(
                                            f"Deleted leftover duplicate: {obj_name}")
                                    except BaseException:
                                        pass
                        ...
                        with state_manager.ScopedIsolation(context, target, include_subcollections=start_point.include_subcollections if start_point.start_point_type == 'COLLECTION' else True) as isolation:
                            logger.log_step(
                                "isolation_started", start_point_context)
                            ...
                            sanitized_name = re.sub(
                                r'[^a-zA-Z0-9_]', '_', start_point.name)
                            # Remove leading/trailing underscores and multiple
                            # consecutive underscores
                            sanitized_name = re.sub(
                                r'_+', '_', sanitized_name).strip('_')
                            ...
                            # Reference: NVIDIA_BLender_BestPractise.md section
                            # 6.1
                            duplicated_objects = []  # Track duplicated objects for cleanup
                            # Track all mesh objects for export (populated for
                            # collections)
                            mesh_objects = []
                            # Track name swaps for rollback: {hidden_name:
                            # original_name}
                            ...
                            if start_point.start_point_type == 'COLLECTION':
                                pivot_only = getattr(
                                    start_point, "collection_pivot_only", False)
                                collection_needs_normalization = (
                                    getattr(
                                        ...
                                        start_point,
                                        "normalize_rotation",
                                        False) or pivot_only or _collection_pivot_enabled(start_point))
                                        ...
                                        json.dumps(
                                            {
                                                ...
                                                "runId": "run1",
                                                "hypothesisId": "A",
                                                "location": "ops_export.py:353",
                                                "message": "duplicated_objects initialized",
                                                "data": {
                                                    "start_point_name": start_point.name,
                                                    "duplicated_objects_count": 0},
                            ...
                            # Collection export: check export_modifiers OR
                            # collection normalization
                            should_process_modifiers = (start_point.export_modifiers or (
                                start_point.start_point_type == 'COLLECTION' and collection_needs_normalization))
                            ...
                            if should_process_modifiers:
                                ...
                                # But we duplicate objects first to preserve
                                # originals (like CTRL+Z after export)
                                ...
                                if start_point.start_point_type == 'OBJECT':
                                    ...
                                    if obj and obj.type == 'MESH':
                                        ...
                                        # Previous bug: Objects like
                                        # "Kayak.002" would switch to "Kayak"
                                        # which might not have modifiers
                                        obj_name = obj.name

                                        # v0.1.30: Log modifier check for
                                        # debugging
                                        modifier_names = [
                                        ...
                                        # The should_apply_modifier() function
                                        # will handle visibility filtering
                                        # later
                                        ...
                                        if needs_duplication:
                                            ...
                                            # 5. Delete duplicate, rename
                                            # original back

                                            original_name = obj.name
                                            hidden_name = f"__USDME_ORIG_{original_name}"

                                            # Step 1: Rename original to hidden
                                            # name
                                            try:
                                            ...
                                            # Step 2: Duplicate (should get
                                            # original_name since it's now
                                            # free)
                                            try:
                                                # v0.1.14: Get original mesh
                                                # data name BEFORE duplication
                                                original_mesh_name = None
                                                original_mesh = None
                                                if obj.data and hasattr(
                                                        obj.data, 'name'):
                                                ...
                                                # The duplicate's mesh data
                                                # also needs the original name
                                                # for USD consistency
                                                ...
                                                # Link duplicate to same
                                                # collection(s) as original
                                                        ...
                                                        json.dumps(
                                                            {
                                                                ...
                                                                "runId": "run1",
                                                                "hypothesisId": "C",
                                                                "location": "ops_export.py:381",
                                                                "message": "duplicate created for OBJECT",
                                                                "data": {
                                                                    "start_point_name": start_point.name,
                                                                    "original_name": obj.name,
                                                                    "duplicate_name": dup_obj.name,
                                                                    "duplicated_objects_count": len(duplicated_objects),
                                                                    "reason": ", ".join(reason)},
                                                                "timestamp": __import__("time").time() *
                                                                1000}) +
                                                        "\n")
                                            ...
                                            if start_point.export_modifiers:
                                                # Remove shape keys from
                                                # duplicate first (NVIDIA
                                                # pattern)
                                                ...
                                                # v0.1.13: Track target
                                                # visibility for restoration
                                                target_visibility_backups = {}
                                                modifiers_applied = []

                                                # Apply modifiers in stack order (top to bottom)
                                                # dup_obj.modifiers iterates in
                                                # stack order from top to
                                                # bottom
                                                for mod in list(
                                                        dup_obj.modifiers):
                                                    # v0.1.15: Skip
                                                    ...
                                                    # modifiers
                                                    if not should_apply_modifier(
                                                            mod, logger, start_point_context):
                                                        continue
                                                    ...
                                                    try:
                                                        ...
                                                        # v0.1.13: Force-enable
                                                        # modifier viewport
                                                        # visibility
                                                        ensure_modifier_visibility(
                                                            mod, logger, start_point_context)
                                                        ...
                                                        # modifier types with
                                                        # targets
                                                        ensure_modifier_target_visibility(
                                                            context, mod, target_visibility_backups, logger, start_point_context)
                                                        ...
                                                        bpy.ops.object.modifier_apply(
                                                            modifier=mod.name, single_user=True)
                                                ...
                                                # v0.1.13: Restore target
                                                # visibility after all
                                                # modifiers applied
                                                restore_target_visibility(
                                                    context, target_visibility_backups)
                                                ...
                                                if modifiers_applied:
                                                    ...
                                                    # Log if export_modifiers
                                                    # is enabled but no
                                                    # modifiers found/applied
                                                    logger.warning(
                                    ...
                                    if collection:
                                        ...
                                        def get_safe_collection_objects(
                                                coll, include_children=True):
                                            ...
                                            if include_children:
                                                try:
                                                    for child_coll in coll.children:
                                                        try:
                                                            objects.extend(get_safe_collection_objects(
                                                                child_coll, include_children))
                                                        except Exception:
                                                            continue
                                                except Exception:
                                                    pass
                                        ...
                                        include_subs = start_point.include_subcollections if hasattr(
                                            start_point, 'include_subcollections') else True
                                        objects_to_process = get_safe_collection_objects(
                                            collection, include_children=include_subs)
                                        ...
                                        # Track name swaps for rollback:
                                        # {hidden_name: original_name}
                                        name_swaps = {}
                                        # v0.1.14: Track mesh data name swaps
                                        # for cleanup
                                        mesh_swaps = {}
                                        ...
                                        for obj in mesh_objects:
                                            try:
                                                ...
                                                # Check if this object needs
                                                # modifier baking
                                                needs_duplication = False
                                                reason = []

                                                # v0.1.16: Unified modifier check - includes ALL modifiers (including subdivision)
                                                # v0.1.81+: Also duplicate for
                                                # collection
                                                # normalization/pivot quick-fix
                                                # (non-destructive)
                                                if start_point.export_modifiers:
                                                    try:
                                                        if any(
                                                        ...
                                                        continue
                                                if collection_needs_normalization:
                                                    needs_duplication = True
                                                    reason.append(
                                                        "normalize/pivot")
                                                ...
                                                if needs_duplication:
                                                    ...
                                                    # Step 2: Duplicate (should
                                                    # get original_name since
                                                    # it's now free)
                                                    try:
                                                        # v0.1.14: Get original
                                                        # mesh data name BEFORE
                                                        # duplication
                                                        original_mesh_name = None
                                                        original_mesh = None
                                                        if obj.data and hasattr(
                                                        ...
                                                        # v0.1.14: MESH DATA NAME-SWAP STRATEGY
                                                        # The duplicate's mesh
                                                        # data also needs the
                                                        # original name for USD
                                                        # consistency
                                                        if original_mesh_name and dup_obj.data and dup_obj.data != original_mesh:
                                                            dup_mesh = dup_obj.data
                                                            dup_mesh_name = dup_mesh.name  # e.g., "Plane.011"
                                                        ...
                                                        try:
                                                            ...
                                                            pass
                                                        logger.warning(
                                                            f"Failed to duplicate '{original_name}': {e}", extra=start_point_context)
                                                        continue
                                                    ...
                                                    if start_point.export_modifiers:
                                                        ...
                                                        for mod in list(
                                                            ...
                                                            try:
                                                                ...
                                                                # types
                                                                ensure_modifier_target_visibility(
                                                                    context, mod, coll_target_visibility_backups, logger, start_point_context)
                                                                ...
                                                                context.view_layer.objects.active = dup_obj
                                                                bpy.ops.object.modifier_apply(
                                                                    modifier=mod_name, single_user=True)
                                                        ...
                                                        # applied
                                                        restore_target_visibility(
                                                            context, coll_target_visibility_backups)

                                            except (ReferenceError, AttributeError, RuntimeError) as e:
                                                logger.warning(
                                                    f"Object became invalid during processing: {e}", extra=start_point_context)
                                                continue
                            ...
                            # Capture visibility state before we start
                            # modifying
                            visibility_snapshot = {
                            ...
                            # Build set of original objects that were
                            # duplicated (now hidden as __USDME_ORIG_*)
                            duplicated_original_names = set()
                            for dup_info in duplicated_objects:
                                original_name = dup_info.get("original_name")
                                if original_name:
                                    duplicated_original_names.add(
                                        original_name)
                            ...
                            # 3. For OBJECT type: original object if it wasn't
                            # duplicated
                            objects_to_export = []
                            ...
                            for dup_info in duplicated_objects:
                                ...
                                if dup_obj and dup_obj.name in context.view_layer.objects:
                                    objects_to_export.append(
                                        context.view_layer.objects[dup_obj.name])

                            # Add original objects that weren't duplicated (no modifier baking needed)
                            # For COLLECTION type: These are mesh objects from
                            # the collection
                            for obj in mesh_objects:
                                try:
                                    if obj and obj.name in bpy.data.objects:
                                        # Skip if this object was duplicated
                                        # (it's now named __USDME_ORIG_*)
                                        if obj.name in duplicated_original_names:
                                            continue
                                        # Skip if it's a hidden original
                                        # (starts with __USDME_ORIG_)
                                        if obj.name.startswith(
                                                "__USDME_ORIG_"):
                                            continue
                                        # This object wasn't duplicated,
                                        # include it directly
                                        if obj.name in context.view_layer.objects:
                                            objects_to_export.append(
                                                context.view_layer.objects[obj.name])
                                except (ReferenceError, AttributeError, RuntimeError):
                                    continue

                            # v0.1.16: For OBJECT type, ensure the object is always in objects_to_export
                            # FIXED: Always check for OBJECT type and ensure
                            # object is added
                            if start_point.start_point_type == 'OBJECT':
                                target_obj_name = start_point.object_name
                                obj_already_added = False

                                # Check if object (or its duplicate) is already
                                # in the list
                                for exp_obj in objects_to_export:
                                    # Check if this is the target object or a
                                    # duplicate of it
                                ...
                                if not obj_already_added:
                                    ...
                                    if obj and obj.type == 'MESH' and obj.name in context.view_layer.objects:
                                        # Only add if it's not hidden as
                                        # __USDME_ORIG_*
                                        ...
                                        # Object might be hidden, try to find
                                        # it
                                        logger.warning(
                            ...
                            # Object references can become stale after
                            # view_layer.update() or visibility changes
                            object_names_to_export = [
                                o.name for o in objects_to_export if o and hasattr(
                                    o, 'name')]
                            ...
                            # Collection export: v0.1.82 normalization on exact
                            # export set (origin_set + transform_apply on dupes)
                            if (start_point.start_point_type == 'COLLECTION'
                                    and collection_needs_normalization
                                    and objects_to_export):
                                _apply_collection_quick_normalize(
                                    context=context,
                                    ...
                                    and collection_needs_normalization
                                    and not objects_to_export):
                                logger.warning(
                                    "Collection normalization requested but no "
                                    "objects to export; skipping.",
                                    extra=start_point_context,
                                )

                            # Collection export: Use fake parent approach
                            # (avoids modifying geometry directly)
                            fake_parent = None
                            fake_parent_backups = {}
                            baked_transforms = {}  # Store baked transforms for restoration
                            if start_point.start_point_type == 'COLLECTION':
                                # Create fake parent and parent collection objects to it
                                # This provides transform hierarchy without
                                # modifying geometry (prevents broken normals)
                                ...
                                # the export preserves the correct spatial
                                # relationships.
                                if fake_parent:
                            ...
                            # Collection normalization is handled above for
                            ...
                            # Y-up conversion will be handled in the
                            # post-export USD bake step.
                            # {obj_name: {"location": Vector, "rotation_euler": Euler, "scale": Vector}}
                            transform_backups = {}
                            ...
                            # Fake parent baking already handles all transforms
                            # correctly
                            skip_yup_transform = (
                                start_point.start_point_type == 'COLLECTION' and fake_parent is not None)
                            # Only transform if Y-up is needed and not skipped
                            needs_transform = start_point.y_is_up and not skip_yup_transform
                            ...
                            if needs_transform and object_names_to_export:
                                ...
                                # Use object names and get fresh references
                                # from view_layer
                                for obj_name in object_names_to_export:
                                    if obj_name in context.view_layer.objects:
                                        ...
                                        # 2. Then apply scale to the rotated
                                        # location
                                        ...
                                        # TODO: Re-enable pre-export rotation
                                        # if metadata-only approach doesn't
                                        # work
                                        ...
                                        # Unit conversion scales positions
                                        # (2700mm → 2.7m) but geometry scale
                                        # stays (1,1,1)
                                        if scale_factor != 1.0:
                                        ...
                                        # Keep original scale (don't modify
                                        # obj.scale for unit conversion)
                            ...
                            if object_names_to_export:
                                ...
                                # Step 1: Deselect all objects (while they're
                                # still visible)
                                for obj in context.view_layer.objects:
                                    try:
                                ...
                                # Step 2: Ensure our target objects are
                                # visible, selected, and one is active
                                first_obj_set = False
                                for obj_name in object_names_to_export:
                                    if obj_name in context.view_layer.objects:
                                        ...
                                        # Also unhide in outliner
                                        view_layer_obj.hide_set(False)
                                        # Set as active object first (required
                                        # for selection to work reliably)
                                ...
                                # Step 4: NOW hide all other objects (after
                                # selection is done)
                                for obj in context.view_layer.objects:
                                ...
                                # Fallback: no objects to export, use
                            ...
                            if start_point.start_point_type == 'COLLECTION':
                                ...
                                # For OBJECT type, use selected_objects_only to
                                # export only the isolated object
                                export_params["selected_objects_only"] = True
                            ...
                            # This is in addition to export_lights=False
                            # parameter
                            lights_deselected = []
                            ...
                            # Capture any error messages from Blender's
                            # operator
                            try:
                                result = bpy.ops.wm.usd_export(**export_params)

                                # Check for error messages in Blender's
                                # operator reports
                                error_messages = []
                                if hasattr(
                                        bpy.context, 'operator_properties_last'):
                                ...
                                logger.log_error(
                                    ...
                                    recoverable=True)
                                self.report(
                                    {"ERROR"}, f"Failed to export '{start_point.name}': {str(export_error)}")
                                ...
                                if result == {"FINISHED"}:
                                    ...
                                    logger.log_step(
                                        "usd_export_success", {
                                            **start_point_context, "file_size_bytes": self._get_file_size(filepath)})

                                    # Add origin metadata if enabled
                                    if start_point.include_origin_metadata:
                                        self._add_origin_metadata(
                                            filepath, start_point, logger, start_point_context)
                                    ...
                                    # Baking transforms into geometry ensures
                                    # consistent behavior.
                                    self._apply_stage_metadata(
                                        filepath, start_point, logger, start_point_context)

                                    self.report(
                                        {"INFO"}, f"Successfully exported '{start_point.name}' to '{filepath}'.")
                                else:
                                    failed_count += 1
                                    ...
                                    logger.log_error(
                                        ...
                                        recoverable=True)
                                    self.report(
                                        {"ERROR"}, f"Failed to export '{start_point.name}'. Check console for details.")

                            # Restore transforms if they were applied
                            if transform_backups:
                                logger.log_step(
                                    "transform_restore_start", {
                                        **start_point_context, "transforms_to_restore": len(transform_backups)})
                                ...
                                for obj_name, backup in transform_backups.items():
                                    ...
                                    if obj_ref and obj_ref.name in context.view_layer.objects:
                                        try:
                                            ...
                                            logger.log_step(
                                                "transform_restored", {
                                                    **start_point_context, "object_name": obj_name})
                                        except Exception as e:
                                            logger.warning(
                            ...
                            # Restore visibility state so the rest of the scene
                            # returns to normal
                            ...
                            # Must happen BEFORE cleanup to restore parent
                            # relationships
                            ...
                            # Must happen BEFORE ScopedIsolation exits to
                            ...
                            # This prevents ScopedIsolation from trying to
                            # restore state using deleted objects
                            duplicated_objects_for_cleanup.extend(
                                duplicated_objects)
                            ...
                            # v0.1.17: Store name swaps for cleanup (OBJECT and
                            # COLLECTION types)
                        ...
                        if duplicated_objects_for_cleanup:
                            ...
                            try:
                                ...
                                for dup_info in duplicated_objects_for_cleanup:
                                    dup_obj = dup_info.get("duplicate")
                                    if dup_obj:
                                        # Find the duplicate by its current
                                        # name (which is the original name
                                        # after swap)
                                        dup_ref = bpy.data.objects.get(
                                            dup_obj.name)
                                        if dup_ref:
                                ...
                                if mesh_swaps_for_cleanup:
                                    for hidden_mesh_name, swap_info in mesh_swaps_for_cleanup.items():
                                        original_mesh_name = swap_info.get(
                                            'original_name')
                                        original_mesh = swap_info.get(
                                            'original_mesh')
                                        ...
                                        if original_mesh and original_mesh.name in bpy.data.meshes:
                                            try:
                                                # Find the mesh by its hidden
                                                # name
                                                mesh_ref = bpy.data.meshes.get(
                                                    hidden_mesh_name)
                                                if mesh_ref:
                                    ...
                                    json.dumps(
                                        {
                                            ...
                                            "data": {
                                                ...
                                                "exception_msg": str(e)[
                                                    :100],
                                                "duplicated_objects_count": len(duplicated_objects) if 'duplicated_objects' in locals() else 0},
                                            "timestamp": __import__("time").time() *
                                            1000}) +
                                    "\n")
                        except BaseException:
                        ...
                        if 'baked_transforms' in locals(
                        ) and baked_transforms and 'fake_parent' in locals() and fake_parent:
                            try:
                                _restore_baked_transforms(
                                    context=context,
                                    baked_transforms=baked_transforms,
                        ...
                        logger.log_error(
                            ...
                            recoverable=False)
                        self.report(
                            {"ERROR"}, f"Error exporting '{start_point.name}': {str(e)}")
        ...
        if success:
            ...
            self.report(
                {"INFO"},
                f"USD Multi Export completed: {exported_count} successful, {failed_count} failed.")
        else:
        ...
        try:
            ...
            if root_prim.IsValid():
                ...
                root_prim.CreateAttribute(
                    "usdme:origin_file", Sdf.ValueTypeNames.String).Set(
                    blend_filepath if blend_filepath else "Unsaved")
                root_prim.CreateAttribute(
                    "usdme:origin_filename", Sdf.ValueTypeNames.String).Set(
                    os.path.basename(blend_filepath) if blend_filepath else "Unsaved")
                root_prim.CreateAttribute(
                    "usdme:origin_username",
                    Sdf.ValueTypeNames.String).Set(
                    getpass.getuser())
                root_prim.CreateAttribute(
                    "usdme:origin_computer",
                    Sdf.ValueTypeNames.String).Set(
                    socket.gethostname())
                root_prim.CreateAttribute(
                    "usdme:export_timestamp",
                    Sdf.ValueTypeNames.String).Set(
    ...
    def _apply_stage_metadata(
        ...
        try:
            ...
            scale_factor = start_point.get_scale_factor() if hasattr(
                start_point, 'get_scale_factor') else 1.0

            # v0.1.84: When user requested normalize rotation/scale, also normalize
            # child prim xforms in USD (bake into mesh, set to identity).
            # v0.1.86+: Normalize Scale/Rotation disabled (upcoming) — breaks collection
            # decals and post-export bake logic. See 04_Implementation_Plan.md.
            normalize_child_xforms = False
            bake_result = usd_bake.bake_usd_geometry(
                ...
                logger=logger,
                start_point_context=start_point_context,
                normalize_child_xforms=normalize_child_xforms,
            )
            ...
            # These have the same base name but random suffixes like .1dqzwk,
            # .634rf1
            import os
            import glob
    ...
    def _apply_collection_root_transforms(
            self,
            filepath: str,
            start_point,
            logger,
        ...
        try:
            ...
            # not for unit conversion. We need to calculate the conversion
            # ourselves.
            unit_scale = {
            ...
            # we need to multiply by 100 to convert meter values to centimeter
            # values
            ...
            if not stage:
                logger.log_error(
                    "Could not open stage for root transform application",
                    extra=start_point_context)
                return

            # v0.1.68 FIX: Use default prim directly instead of trying to
            # construct path
            root_prim = stage.GetDefaultPrim()

            if not root_prim or not root_prim.IsValid():
                logger.warning(
                    "Could not find root prim for transform application",
                    extra=start_point_context)
                return
            ...
            # This is because USD applies transforms in order: first in list
            # applies first
